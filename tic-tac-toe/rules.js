...
Usage: This is to store the state to the tic-tac-toe board.
When a move is made 
...
function begin_play() {
    ...
    if (isEmpty(p1.value) || isEmpty(p2.value)) {
        ...
        p1.value = player1.value + "(X)";
        p2.value = player2.value + "(O)";
...
function play() {
    //1
    if (!document.getElementById('move_text_id').value.match(/[A-C][1-3]/g)) {
        //2
        alert("Invalid move!");
        return;
    }
    ...
    if (!game_started()) {
        ...
        //TODO: update grid if move valid
        //TODO: validate table & reset if winner
    }
}
